...
#include "PhaseCorrModernized.h"

#define CV_MAKETYPE2(depth,cn) (CV_MAT_DEPTH2(depth) + (((cn)-1) << 3))
#define CV_MAT_DEPTH2(flags)     ((flags) & ((1 << 3) - 1))
#define CV_MAT_CN2(flags)        ((((flags) & ((512 - 1) << 3) ) >> 3) + 1)
#define CV_SWAP2(a,b,t) ((t) = (a), (a) = (b), (b) = (t))

#define BitRev2(i,shift) \
    ((int)((((unsigned)bitrevTab2[(i)&255] << 24)+ \
    ((unsigned)bitrevTab2[((i)>> 8)&255] << 16)+ \
    ((unsigned)bitrevTab2[((i)>>16)&255] <<  8)+ \
    ((unsigned)bitrevTab2[((i)>>24)])) >> (shift)))
...
DFTFactorize(int n, int* factors)
{
    ...
    f = (factors[0] & 1) == 0;
    for (i = f; i < (nf + f) / 2; i++)
        CV_SWAP2(factors[i], factors[nf - i - 1 + f], j);

    return nf;
}

constexpr unsigned char bitrevTab2[] =
{
...
constexpr const double DFTTab2[][2] =
{
...
DFTInit(int n0, int nf, int* factors, int* itab, int elem_size, void* _wave, int inv_itab)
{
    ...
    if ((n0 & (n0 - 1)) == 0)
    {
        w.re = w1.re = DFTTab2[m][0];
        w.im = w1.im = -DFTTab2[m][1];
    }
    else
    {
        t = -CV_PI * 2 / n0;
...
template<typename T> struct DFT_VecR4_2
{
...
DFT2(const Complex<T>* src, Complex<T>* dst, int n,
...
template<typename T> static void
RealDFT2(const T* src, T* dst, int n, int nf, int* factors, const int* itab,
         ...
         Complex<T>* buf, int flags, double _scale)
{
    ...
    else if (n & 1)
    {
        ...
        DFT2(_dst, _dst, n, nf, factors, itab, wave,
    ...
    else
    {
        ...
        DFT2((Complex<T>*)src, (Complex<T>*)dst, n2, nf - (factors[0] == 1),
                factors + (factors[0] == 1),
...
{
    DFT2(src, dst, n, nf, factors, itab, wave, tab_size, spec, buf, flags, scale);
}
...
{
    RealDFT2(src, dst, n, nf, factors, itab, wave, tab_size, spec, buf, flags, scale);
}
...
template<typename T> static void
CCSIDFT2(const T* src, T* dst, int n, int nf, int* factors, const int* itab,
         const Complex<T>* wave, int tab_size,
         const void*, Complex<T>* buf,
         int flags, double _scale)
{
    ...
    else
    {
        ...
        factors[0] >>= 1;
        DFT2((Complex<T>*)dst, (Complex<T>*)dst, n2,
             nf - (factors[0] == 1),
...
static void CCSIDFT_64f(const double* src, double* dst, int n, int nf, int* factors,
                        ...
                        Complexd* buf, int flags, double scale)
{
    CCSIDFT2(src, dst, n, nf, factors, itab, wave, tab_size, spec, buf, flags, scale);
}

static void complementComplexOutput2(Mat& dst, int len, int dft_dims)
{
    int i, n = dst.cols;
...
static void
ExpandCCS2(uchar* _ptr, int n, int elem_size)
{
    int i;
...
static void
CopyFrom2Columns2(const uchar* _src, size_t src_step,
...
static void
CopyColumn2(const uchar* _src, size_t src_step,
...
static void
CopyTo2Columns2(const uchar* _src0, const uchar* _src1,
                uchar* _dst, size_t dst_step,
                int len, size_t elem_size)
{
    int i, t0, t1;
...
template<typename _Tp> static inline _Tp* alignPtr2(_Tp* ptr, int n = (int)sizeof(_Tp))
{
    return (_Tp*)(((size_t)ptr + n - 1) & -n);
}

void dft2(InputArray _src0, OutputArray _dst, int flags, int nonzero_rows)
{
    ...
    if (!inv && src.channels() == 1 && (flags & DFT_COMPLEX_OUTPUT))
        _dst.create(src.size(), CV_MAKETYPE2(depth, 2));
    ...
    for (;;)
    {
        ...
        if (!spec)
        {
            ...
            itab = (int*)ptr;
            ptr = (uchar*)alignPtr2(ptr + len * sizeof(int), 16);
        ...
        if (stage == 0)
        {
            ...
            if (stage != 1)
            {
                if (!inv && real_transform && dst.channels() == 2)
                    complementComplexOutput2(dst, nonzero_rows, 1);
                break;
            }
            src = dst;
        }
        else
        {
            ...
            if (real_transform)
            {
                ...
                if (!inv)
                {
                    ...
                    CopyColumn2(sptr0, src.step, buf0, complex_elem_size, len, elem_size);
                    sptr0 += dst.channels()*elem_size;
                    if (even)
                    {
                        memset(buf1, 0, len*complex_elem_size);
                        CopyColumn2(sptr0 + (count - 2)*elem_size, src.step,
                                    buf1, complex_elem_size, len, elem_size);
                    }
                }
                else if (src.channels() == 1)
                {
                    CopyColumn2(sptr0, src.step, buf0, elem_size, len, elem_size);
                    ExpandCCS2(buf0, len, elem_size);
                    if (even)
                    {
                        CopyColumn2(sptr0 + (count - 1)*elem_size, src.step,
                                    buf1, elem_size, len, elem_size);
                        ExpandCCS2(buf1, len, elem_size);
                    }
                    sptr0 += elem_size;
                }
                else
                {
                    CopyColumn2(sptr0, src.step, buf0, complex_elem_size, len, complex_elem_size);
                    if (even)
                    {
                        CopyColumn2(sptr0 + b*complex_elem_size, src.step,
                ...
                if (dst.channels() == 1)
                {
                    if (!inv)
                    {
                        ...
                        CopyColumn2(dbuf0 + elem_size, elem_size, dptr0,
                                    dst.step, len, elem_size);
                        if (even)
                        {
                            memcpy(dbuf1 + elem_size, dbuf1, elem_size);
                            CopyColumn2(dbuf1 + elem_size, elem_size,
                    ...
                    else
                    {
                        ...
                        CopyColumn2(dbuf0, complex_elem_size, dptr0, dst.step, len, elem_size);
                        if (even)
                            CopyColumn2(dbuf1, complex_elem_size, dptr0 + (count - 1)*elem_size,
                                        dst.step, len, elem_size);
                        dptr0 += elem_size;
                    }
                }
                ...
                {
                    assert(!inv);
                    CopyColumn2(dbuf0, complex_elem_size, dptr0,
                                dst.step, len, complex_elem_size);
                    if (even)
                        CopyColumn2(dbuf1, complex_elem_size,
            ...
            for (i = a; i < b; i += 2)
            {
                if (i + 1 < b)
                {
                    CopyFrom2Columns2(sptr0, src.step, buf0, buf1, len, complex_elem_size);
                    dft_func(buf1, dbuf1, len, nf, factors, itab,
                             wave, len, spec, ptr, inv, scale);
                }
                else
                ...
                if (i + 1 < b)
                    CopyTo2Columns2(dbuf0, dbuf1, dptr0, dst.step, len, complex_elem_size);
                else
                    CopyColumn2(dbuf0, complex_elem_size, dptr0, dst.step, len, complex_elem_size);
            ...
            if (stage != 0)
            {
                if (!inv && real_transform && dst.channels() == 2 && len > 1)
                    complementComplexOutput2(dst, len, 2);
                break;
            }
            src = dst;
        }
...
void idft2(const InputArray &src, const OutputArray &dst, int flags, int nonzero_rows)
{
    dft2(src, dst, flags | DFT_INVERSE, nonzero_rows);
}



template<typename T, typename WT> static void
minMaxIdx_2(const T* src, const uchar* mask, WT* _minVal, WT* _maxVal,
...
{
    minMaxIdx_2(src, mask, minval, maxval, minidx, maxidx, len, startidx);
}
...
typedef void(*MinMaxIdxFunc2)(const uchar*, const uchar*, int*, int*, size_t*, size_t*, int, size_t);

static MinMaxIdxFunc2 getMinmaxTab2(int depth)
{
    static MinMaxIdxFunc2 minmaxTab[] =
    {
        (MinMaxIdxFunc2)(minMaxIdx_8u), (MinMaxIdxFunc2)(minMaxIdx_8s),
        (MinMaxIdxFunc2)(minMaxIdx_16u), (MinMaxIdxFunc2)(minMaxIdx_16s),
        (MinMaxIdxFunc2)(minMaxIdx_32s),
        (MinMaxIdxFunc2)(minMaxIdx_32f), (MinMaxIdxFunc2)(minMaxIdx_64f),
        0
    };
...
void minMaxIdx2(InputArray _src, double* minVal, double* maxVal, int* minIdx, int* maxIdx, InputArray _mask )
{
    ...
    MinMaxIdxFunc2 func = getMinmaxTab2(depth);
    CV_Assert(func != 0);
...
void minMaxLoc2(InputArray _img, double* minVal, double* maxVal, Point* minLoc, Point* maxLoc, InputArray mask = noArray() )
{
    CV_Assert(_img.dims() <= 2);

    minMaxIdx2(_img, minVal, maxVal, (int*)minLoc, (int*)maxLoc, mask);
    if (minLoc)
...
void mulSpectrums2(InputArray _srcA, InputArray _srcB, OutputArray _dst, int flags, bool conjB)
{
...
void multiply2(Mat src1, Mat src2,	Mat dst)
{
    for (int i = 0; i < src1.rows; i++)
        for (int j = 0; j < src1.cols; j++)
            dst.at<double>(i, j) = src1.at<double>(i, j) * src2.at<double>(i, j);// src1 * src2;
}
...
template<typename _Tp, size_t fixed_size> inline
AutoBuffer2<_Tp, fixed_size>::operator _Tp* ()
{
    return ptr;
}

int borderInterpolate2(int p, int len, int borderType)
{
    if ((unsigned)p < (unsigned)len)
        ;
    else if (borderType == BORDER_REPLICATE2)
        p = p < 0 ? 0 : len - 1;
    else if (borderType == BORDER_REFLECT2 || borderType == BORDER_REFLECT_101_2)
    {
        int delta = borderType == BORDER_REFLECT_101_2;
        if (len == 1)
            return 0;
        do
        {
    ...
    else if (borderType == BORDER_WRAP2)
    {
        CV_Assert(len > 0);
        if (p < 0)
            p -= ((p - len + 1) / len)*len;
    ...
    else if (borderType == BORDER_CONSTANT2)
        p = -1;
    else
                       ...
                       int top, int left, int cn, int borderType)
{
    ...
    AutoBuffer2<int> _tab((dstroi.width - srcroi.width)*cn);
    int* tab = _tab;
    ...
    for (i = 0; i < left; i++)
    {
        j = borderInterpolate2(i - left, srcroi.width, borderType)*cn;
        for (k = 0; k < cn; k++)
            tab[i*cn + k] = j + k;
    }
    ...
    for (i = 0; i < right; i++)
    {
        j = borderInterpolate2(srcroi.width + i, srcroi.width, borderType)*cn;
        for (k = 0; k < cn; k++)
            tab[(i + left)*cn + k] = j + k;
    }
    ...
    for (i = 0; i < top; i++)
    {
        j = borderInterpolate2(i - top, srcroi.height, borderType);
        memcpy(dst + (i - top)*dststep, dst + j*dststep, dstroi.width);
    }

    for (i = 0; i < bottom; i++)
    {
        j = borderInterpolate2(i + srcroi.height, srcroi.height, borderType);
...
{
    int i, j;
    AutoBuffer2<uchar> _constBuf(dstroi.width*cn);
...
void copyMakeBorder2(InputArray _src, OutputArray _dst, int top, int bottom,
                     int left, int right, int borderType, const Scalar& value)
{
    ...
    if (src.isSubmatrix() && (borderType & BORDER_ISOLATED2) == 0)
    {
        Size wholeSize;
        Point ofs;
    ...
    borderType &= ~BORDER_ISOLATED2;

    if (borderType != BORDER_CONSTANT2)
    ...
    else
    {
        ...
        scalarToRawData(value, buf, CV_MAKETYPE2(src.depth(), cn1), cn);
...
int getOptimalDFTSize2(int size0)
{
...
static void magSpectrums2(InputArray _src, OutputArray _dst)
{
    Mat src = _src.getMat();
    int type = src.type();
...
static void divSpectrums2(InputArray _srcA, InputArray _srcB, OutputArray _dst, int flags, bool conjB)
{
    Mat srcA = _srcA.getMat(), srcB = _srcB.getMat();
    int type = srcA.type();
    int rows = srcA.rows, cols = srcA.cols;
    ...
    const double* dataB = srcB.ptr<double>();
    double* dataC = dst.ptr<double>();
    double eps = EPSILON2; // smallest such that 1.0+DBL_EPSILON != 1.0// prevent div0 problems
...
static void fftShift2(InputOutputArray _out)
{
    Mat out = _out.getMat();
    ...
    Mat tmp;
    Mat q0(out, Rect(0, 0, xMid, yMid));
    Mat q1(out, Rect(xMid, 0, xMid, yMid));
    Mat q2(out, Rect(0, yMid, xMid, yMid));
    Mat q3(out, Rect(xMid, yMid, xMid, yMid));
...
static Point2d weightedCentroid2(InputArray _src, cv::Point peakLocation, cv::Size weightBoxSize, double* response)
{
    Mat src = _src.getMat();
    ...
    sumIntensity += EPSILON2; // prevent div0 problems...
...
void ShowDepthMap(const Mat &map,  int *countPeaks)
{
    double minVal, maxVal;
    minMaxLoc(map, &minVal, &maxVal);
    Mat adjMap;
    double alpha = 255.0 / (maxVal - minVal);
    double beta = -minVal * alpha;
    map.convertTo(adjMap, CV_8UC1, alpha, -minVal * beta);

    Mat thresholdAdjMap;
    threshold(adjMap, thresholdAdjMap, 230, 255, THRESH_TOZERO);
    //imshow("DepthMap_Thr", thresholdAdjMap);
    ...
    binaryImage = cvCreateImage(cvSize(thresholdAdjMap.cols,thresholdAdjMap.rows), 8, 1);
    IplImage ipltemp = thresholdAdjMap;
    cvCopy(&ipltemp, binaryImage);

    *countPeaks = cvFindContours(binaryImage, _storage, &contours, sizeof(CvContour), CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE, cvPoint(0, 0));
}

ResultPhaseCorrelation phaseCorrelate2(const InputArray &_src1, const InputArray &_src2, const InputArray &_window)
{
    ...
    int M = getOptimalDFTSize2(src1.rows);
    int N = getOptimalDFTSize2(src1.cols);

    Mat padded1, padded2, paddedWin;

    if (M != src1.rows || N != src1.cols)
    {
        copyMakeBorder2(src1, padded1, 0, M - src1.rows, 0, N - src1.cols, BORDER_CONSTANT, Scalar::all(0));
        copyMakeBorder2(src2, padded2, 0, M - src2.rows, 0, N - src2.cols, BORDER_CONSTANT, Scalar::all(0));
    ...
    if (!paddedWin.empty())
    {
        ...
        multiply2(paddedWin, padded1, padded1);
        multiply2(paddedWin, padded2, padded2);
    }
    ...
    mulSpectrums2(FFT1, FFT2, P, 0, true);

    magSpectrums2(P, Pm);
    divSpectrums2(P, Pm, C, 0, false); // FF* / |FF*| (phase correlation equation completed here...)

    idft2(C, C); // gives us the nice peak shift location...

    fftShift2(C); // shift the energy to the center of the frame.

    Point peakLoc; // locate the highest peak
    double min, max;
    minMaxLoc2(C, &min, &max, NULL, &peakLoc);

    int  countPeaks = 1; //???
    //ShowDepthMap(C, &countPeaks);

    //ResultPhaseCorrelation debugTestResult;
    //return debugTestResult;    //???? for test only !!!! remove after testing


    // get the phase shift with sub-pixel accuracy, 5x5 window seems about right here...
    double response;
    Point2d t;
    t = weightedCentroid2(C, peakLoc, Size(5, 5), &response); //Size(6, 6)
    ...
    Point2d shift = (center - t);

    ResultPhaseCorrelation result;

    result.Shift = shift;
    result.Response = response;
    result.Max = max;
    result.CountPeaks = countPeaks;

    return result;
}

void createHanningWindow2(const OutputArray &_dst, const Size &winSize, int type)
{
    CV_Assert(type == CV_32FC1 || type == CV_64FC1);
    ...
    double coeff0 = 2.0 * CV_PI2 / (double)(cols - 1), coeff1 = 2.0f * CV_PI2 / (double)(rows - 1);
