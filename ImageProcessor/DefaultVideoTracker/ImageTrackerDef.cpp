...
ImageTrackerDef::ImageTrackerDef() : ImageTracker()
{
    _targetSize = 20;
    _targetLocked = false;
}
...
ImageTrackerDef::~ImageTrackerDef()
{
...
QRect ImageTrackerDef::processFrame(const cv::Mat &frame, const FrameShift2D &frameShift)
{
    _frame = frame;
...
void ImageTrackerDef::lockTarget(const QPoint &targetCenter)
{
...
void ImageTrackerDef::unlockTarget()
{
    _targetLocked = false;
}

void ImageTrackerDef::setTargetSize(int size)
{
    _targetSize = size;
    if (_targetLocked)
